...
router.post("/", async ( req, res) => {
    ...
    if (textVal == "") {
        ...
        // Only display if user is a paid member
        response += `CON 1. Receive a call from a health coach\n`;
        response += `2. Register at one of our nudge hubs`
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 2) {
        // store the name of the user
        await utilsController.setRegisterInfo(sessionId, {name: ussd_string_exploded[1]});
        response += `CON Enter your email address? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 3) {
        // store the email of the user
        await utilsController.setRegisterInfo(sessionId, {email: ussd_string_exploded[2]});
        response += `CON Enter your DOB (DD/MM/YYYY)? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 4) {
        // store the dob of the user
        await utilsController.setRegisterInfo(sessionId, {dob: ussd_string_exploded[3]});
        response += `CON Enter your gender (F/M)? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 5) {
        // store the gender of the user
        await utilsController.setRegisterInfo(sessionId, {gender: ussd_string_exploded[4]});
        response += `CON Enter your LGA? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 6) {
        // store the lGA of the user
        await utilsController.setRegisterInfo(sessionId, {lga: ussd_string_exploded[5]});
        response += `CON Enter your height (cm)? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 7) {
        // store the height of the user
        await utilsController.setRegisterInfo(sessionId, {height: ussd_string_exploded[6]});
        response += `CON Enter your weight (kg)? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 8) {
        // store the weight of the user
        await utilsController.setRegisterInfo(sessionId, {weight: ussd_string_exploded[7]});
        response += `CON Enter your waist circumference (cm)? \n`;
        response += footer
    } else if (ussd_string_exploded[0] === "1" && $level === 9) {
        // store the waist circumference of the user
        await utilsController.setRegisterInfo(sessionId, {waist: ussd_string_exploded[8]});
